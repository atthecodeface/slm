...
def set_compile_options(info_struct, kernel_def, downup_sign=1,
    ...
    if job_type=='kde':
        return [
            ...
            '-D','KDF_CODE={}u'.format(info_struct['kdf_code'][0]),
            ...
            '-D','KDF_IS_TOPHAT={}u'.format(info_struct['kdf_is_tophat'][0]),
            '-D','KDF_IS_TRIANGLE={}u'.format(info_struct['kdf_is_triangle'][0]),
            '-D','KDF_IS_EPANECHNIKOV={}u'.format(info_struct['kdf_is_epanechnikov'][0]),
            '-D','KDF_IS_COSINE={}u'.format(info_struct['kdf_is_cosine'][0]),
            '-D','KDF_IS_GAUSSIAN={}u'.format(info_struct['kdf_is_gaussian'][0])
        ]
    else:
        return [
        '-D','KERNEL_{}'.format(kernel_def.upper()),
