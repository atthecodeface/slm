...
def estimate_bivariate_pdf( cl_src_path, which_cl_platform, which_cl_device, 
    ...
    bandwidth    = info_dict['kdf_bandwidth']
    
    x_range      = info_dict['x_range']
    bin_dx       = info_dict['bin_dx']
    pdf_dx       = info_dict['pdf_dx']
    stddev_x     = np.std(sl_array[0,:])
    
    y_range      = info_dict['y_range']
...
#     kdf_width_y = 1.06*stddev_y*np.power(n_data,-0.2)*20
    ...
    info_dict['kdf_width_x'] = kdf_width_x
...
#            info_dict['n_hist_bins']//info_dict['n_pdf_points'])
    ...
    histogram_array \
        = gpu_compute(device, context, queue, cl_kernel_source, cl_kernel_fn, 
                      info_dict, action='histogram',  is_bivariate=True, 
    ...
    partial_pdf_array \
        = gpu_compute(device, context, queue, cl_kernel_source, cl_kernel_fn, 
                      info_dict, action='partial_pdf',  is_bivariate=True, 
...
#     return partial_pdf_array/(np.sum(partial_pdf_array)*bin_dx*bin_dy)
    ...
    pdf_array \
        = gpu_compute(device, context, queue, cl_kernel_source, cl_kernel_fn, 
                      info_dict,  action='full_pdf',  is_bivariate=True, 
...
def estimate_univariate_pdf( cl_src_path, which_cl_platform, which_cl_device, 
    ...
    info_dict['kdf_width_x'] = kdf_width_x
    info_dict['n_kdf_part_points_x'] = np.uint32(np.floor(kdf_width_x/pdf_dx))//2
    ...
    histogram_array \
        = gpu_compute(device, context, queue, cl_kernel_source, cl_kernel_fn, 
                      info_dict, action='histogram', sl_array=sl_array, 
                      verbose=verbose)
    ...
    pdf_array \
        = gpu_compute(device, context, queue, cl_kernel_source, cl_kernel_fn, 
                      info_dict, action='full_pdf', histogram_array=histogram_array,
                      verbose=verbose)
    # Done
    vprint(verbose,'done')
